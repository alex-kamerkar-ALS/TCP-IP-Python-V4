...
def main():
    ...
    try:
        with DobotRobot(ROBOT_IP) as robot:
            ...
            # 查询机器人模式
            mode_response = robot.robot_control.robot_mode()
            print(f"机器人模式: {mode_response}")
            
            while True:
                # 初始化传送带
                print("\n初始化传送带...")
                robot.plugins.cnv_init(1)
                ...
                print("\n等待物体进入抓取区域...")
                while True:
                    cnv_status = robot.plugins.get_cnv_object(0)
                ...
                # 启动同步跟随
                print("\n启动传送带同步...")
                robot.plugins.start_sync_cnv()
                
                # 执行传送带跟随运动
                print("执行传送带跟随运动...")
                robot.plugins.cnv_movl([0, 0, 0, 0, 0, 153])
                time.sleep(2)
                ...
                # 下降抓取
                print("下降抓取...")
                robot.plugins.cnv_movl([0, 0, -50, 0, 0, 153])
                time.sleep(3)
                
                # 停止同步
                print("停止传送带同步...")
                robot.plugins.stop_sync_cnv()
